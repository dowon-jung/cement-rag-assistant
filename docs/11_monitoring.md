...
이 모니터링 설계로 얻을 수 있는 효과는 다음과 같습니다.

1. **수치 기반 의사결정** — 캐시 hit rate, 응답 시간 p95 같은 실제 지표를 보고 최적화 결정
2. **병목 식별** — Jaeger 트레이스로 Agent별 실행 시간을 분석하여 느린 구간 파악
3. **비용 최적화** — LLM 토큰 사용량 추적으로 백엔드별 비용 비교 및 최적 백엔드 선택
4. **실서비스 운영 역량** — HPA 트리거 메트릭이 명확히 정의되어 자동 스케일링 신뢰성 확보
